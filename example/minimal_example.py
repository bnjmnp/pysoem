...
class MinimalExample:
    ...
    def run(self):
        ...
        if self._master.config_init() > 0:
            ...
            try:
                while 1:
                    # ofcourse that's pretty cheap
                    self._master.send_processdata()
                    self._master.receive_processdata(2000)
                    
                    volgage_ch_1_el3002_as_bytes = self._master.slaves[1].input
                    volgage_ch_1_el3002_as_int16 = struct.unpack('hh', volgage_ch_1_el3002_as_bytes)[0]
                    voltage = volgage_ch_1_el3002_as_int16 * 10 / 0x8000
                    print('EL3002 Ch 1 PDO: {:#06x}; Voltage: {:.4}'.format(volgage_ch_1_el3002_as_int16, voltage))
